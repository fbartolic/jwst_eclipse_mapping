...
NIRCAM_FILTERS = "filter_files/F444W_ModAB_mean.csv"
NIRCAM_PATH = os.path.join(HERE, NIRCAM_FILTERS)
...
def readin_nircam_filters(f=NIRCAM_PATH):
    '''
    Function to read-in photometic filters
    ...
    # Read in NIRCAM filter response functions
    df_filt = pd.read_csv(f)
    vals = np.array(df_filt.values[1:,:], dtype=float)
...
class Filter(object):
    ...
    def compute_lightcurve(self, time, flux, continuum, lam, texp=None, stack = 1,
        ...
        :param float texp: Exposure time, same units as Time grid (days).
...
def plot_nircam_filters(ax, wl, filters, names, ylim=(0.0,1.0), leg=True):
    """
    Plot JWST/NIRCAM photometric filter bands.
...
def estimate_eclipse_snr(tint = 36.4*60., nout = 4.0, lammin = 1.0,
    ...
    if type(filters) is list:
        ...
        # Read-in MIRI filters
        wl_filt, dwl_filt, tputs, fnames = readin_nircam_filters()
        nfilt = len(fnames)
    ...
    if plot:
        ...
        if filters == "NIRCAM": plot_nircam_filters(ax, wl_filt, tputs, fnames)
...
def get_nircam_filter_wheel():
    """
    Create a `list` of :py:func:`Filter` objects
